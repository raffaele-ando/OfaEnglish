...
node scripts/contrasto.cjs "#testo:#sfondo" …          # ogni colore nuovo introdotto (testo, min 4,5)
node scripts/contrasto.cjs --grafica "#barra:#traccia" …   # barre, anelli, icone, focus, bordi di input (min 3)
```
...
- [ ] **Grafica ≥ 3:1 in entrambi i temi** (WCAG 1.4.11), anche se il numero è scritto accanto:
  - riempimento di barre e anelli contro la traccia **e** contro la card (verde vivo su traccia grigia = 1,69:1: nel chiaro usa `*-barra` = pieno, 3,79:1);
  - icone che portano significato (✓/✗, icona della metrica) con il tono `*-testo`, non il vivo sul bianco;
  - anello di focus (`outline-focus`), bordi di input (`border-bordo-input`, non `#E5E7EB` a 1,24:1);
  - stato selezionato portato da badge pieno o icona, non dal solo bordo vivo;
  - serie di grafici e segmenti impilati contro quelli accanto.
...
- [ ] Funziona a **320 px** di larghezza senza scroll orizzontale e senza CTA che va a capo; `100dvh` e `safe-area-inset-bottom` rispettati.
- [ ] Su desktop una colonna centrata (`max-w-[480px]`–`max-w-3xl`), non un telefono ad altezza fissa; le pagine scorrono, solo l'app shell è alta quanto la finestra.
- [ ] Il bordo 3D solo su ciò che si tocca; le tile di sola lettura sono piatte.
- [ ] Etichette, descrizioni, pillole e voci di navigazione **≥ 14 px**; 12 px solo per didascalie non essenziali; niente compressione per «far stare tutto».
- [ ] Maiuscolo solo via CSS, solo per etichette di ≤ 2 parole, con `tracking-wide`; CTA in maiuscola iniziale (o una parola corta).
...
- [ ] «Costanza (ultimi 7 giorni)» è un grafico informativo (domande o minuti al giorno): niente contatore di giorni di fila, fiamme sui giorni o giorni persi in rosso; 🔥 solo per le giuste di fila nella sessione.
